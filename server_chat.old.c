...
int main (int argc, char *argv[])
{
    ...
    socklen_t sin_size;
    char client_name[30], client_message[MAXDATASIZE], list_client[MAXCLIENT][30], s[INET6_ADDRSTRLEN];
    int rv;
    struct sigaction sa;
    ...
    while(1) {  // main accept() loop
        ...
        if ((numbytes = recv(new_fd, client_name, MAXDATASIZE-1, 0)) == -1) {
            perror("recv");
            exit(1);
        }
       ...
       // store client's name into list 
        if (MAXCLIENT > c) {
            strcpy(list_client[c], client_name); 
            c++; 
        }
